...
//                 name: 'Not a bomb, but centered on Brazil',
        ...
        //     var testNode = {
        //         "venue": bubbleData[i].venue.displayName,
        //         "children": [bubbleData[i].venue.id
        //         ]
        // };
        //     nodeInfo["venue"] = (testNode);
...
function createTour() {
    ...
    TourTravel.arc(gigRoute, {arcSharpness: .8, strokeWidth: 1});
...
var newNode = null;
// function createNodeData(testNodeData) {
//         var nNode = 50;
//     for (var i = 0; i < nNode; i++){
//         if (i < nNode - 1) {
//         newNode = [
//             { "name": testNodeData[i].name, "parent": "null", "relation": "null", "depth": 1},
//             { "name": testNodeData[i].date, "parent": testNodeData[i].name, "relation": "rel", "depth": 0}
//         ]};
//     console.log("node test");
//     console.log(newNode);
//     }};

// createNodeData(myEventJSON);
// console.log("HERE");
// console.log(newNode);


function drawtreestuff() {
var dataMap = myEventJSON.reduce(function(map, node) {
    map[node.name] = node;
    return map;
}, {});
...
var treeData = [];
myEventJSON.forEach(function(node) {
    // add to parent
...
// make some buttons to drive our zoom
    ...
    d3.select("#btnDiv")[0][0].innerHTML = [].join('\n')


    // ** change buttons here to reflect data from input json file
...
// // future venues
// function loadDataSKVenues3() {
//     d3.json("http://api.songkick.com/api/3.0/venues/{venue_id}/calendar.json?apikey={your_api_key},
//         d3.json("http://api.songkick.com/api/3.0/metro_areas/" + metroAreaIDNY + "/calendar.json?apikey=" + Songkick_APIkey/*+"&min_date=2017-01-01&max_date=2017-01-05"*/
//         , function (error, jsonDataSK) {
//             nResults = Math.round((jsonDataSK.resultsPage.totalEntries) / (jsonDataSK.resultsPage.perPage));
//             for (var i = 1; i < nResults; i++) {
...
// test with webster hall
// venueID = 499281;
// // for a specific venue, get future performances
// function loadDataFuturePerformance () {
//
//     d3.json("http://api.songkick.com/api/3.0/venues/"+venueID+"/calendar.json?apikey="+Songkick_APIkey,
//         function (error, jsonDataSK) {
//             var upcomingLength =  jsonDataSK.resultsPage.results.event.length;
//             for (var i = 0; i < upcomingLength; i++) {
//                 // var venueName = jsonDataSK.resultsPage.results.event[i].venue.displayName;
//                 // var venueID = jsonDataSK.resultsPage.results.event[i].venue.id;
//                 // var bandName = jsonDataSK.resultsPage.results.event[i].performance.displayName;
//
//                 myEventJSON.push({
//                     venueName: jsonDataSK.resultsPage.results.event[i].venue.displayName,
//                     venueID: jsonDataSK.resultsPage.results.event[i].venue.id,
//                     bandName: jsonDataSK.resultsPage.results.event[i].performance.displayName
//                 })
//             }
//
//         })
                ...
                d3.json("http://api.songkick.com/api/3.0/metro_areas/"+metroAreaIDNY+"/calendar.json?apikey="+Songkick_APIkey+"&page="+i, function(error, jsonDataSK2){
                        ...
                        for(var j=0;j<nArrayResults;j++) {
                            ...
                            if (xx != null && yy != null) {
                                ...
                                myEventJSON.push(
                                    {
                                        "name": jsonDataSK2.resultsPage.results.event[j].performance[0].artist.displayName,
                                        "parent": metroAreaIDNY,
                                        "relation": "performer",
                                        "depth": 1
                                    }
                                )
                                ...
                                myEventJSON.push(
                                    {
                                        "name": jsonDataSK2.resultsPage.results.event[j].start.date,
                                        "parent": jsonDataSK2.resultsPage.results.event[j].performance[0].artist.displayName,
                                        "relation": "date",
                                        "depth": 2
                                    }
                                )
...
function loadDataMK() {
    ...
    d3.json("http://api.songkick.com/api/3.0/search/artists.json?query=Chainsmokers&apikey="+Songkick_APIkey,function (error, jsonDataSongkick) {
    ...
    d3.json("https://music-api.musikki.com/v1/artists?q=Chainsmokers&appkey=" + Musikki_AppKey + "&appid=" + Musikki_AppId,
...
$.getJSON("http://api.songkick.com/api/3.0/search/artists.json?query=Chainsmokers&apikey=ME5jCBPTyD3l4BW8",
    ...
    //Request Artis Gigography
        ...
        //Filter In the last Year
        //
            // test with webster 499281
            .defer(d3.json, "http://api.songkick.com/api/3.0/venues/"+venueSKid+".json?apikey="+Songkick_APIkey)
        ...
        .await(function(error,SKdataArtistGigo, SKdataLastYear){
            ...
            // var q = createNodeData(myEventJSON);
            // console.log("blah");
            // console.log(q);
